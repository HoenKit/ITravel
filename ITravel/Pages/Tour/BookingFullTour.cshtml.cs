...
namespace ITravel.Pages.Tour
{
    ...
    public class BookingFullTourModel : PageModel
    {
        ...
        public BookingFullTourModel(IBookingRepository bookingRepository, UserManager<AppUser> userManager, ITourRepository tourRepository, ICustomerRepository customerRepository, IUserRepository userRepository, ILogger<BookingModel> logger, PayOS payOS)
        {
        ...
        public async Task<IActionResult> OnGetAsync(Guid tourId)
        {
            ...
            Price = (int)Math.Ceiling(Tour.Tour.Price * Tour.MaxCapacity * 0.85);
            return Page();
        }
        public async Task<IActionResult> OnPost(Guid tourId, int people)
        {
            ...
            try
            {
                ...
                PaymentRequest.ProductName = "Thanh toán trọn gói - ITravel";
                ...
                PaymentRequest.ReturnUrl = $"http://localhost:5121/Tour/SuccessBooking?tourId={tourId}&action=fullTour";

                ItemData item = new ItemData(PaymentRequest.ProductName, People, Price);
